...
import { fourQuestionsForTopic } from "../../data/questions/generateDefensiveQuestions";
import { getRandomTopic, SEVERITY_LABEL, SEVERITY_COLOR } from "../../data/defensive";

export default function FeaturedDefensiveQuiz() {
  const [round, setRound] = useState(0);
  const [correctCount, setCorrectCount] = useState(0);
  const [answeredCount, setAnsweredCount] = useState(0);

  const topic = useMemo(() => getRandomTopic(), [round]);
  const questions = useMemo(() => fourQuestionsForTopic(topic), [topic]);
