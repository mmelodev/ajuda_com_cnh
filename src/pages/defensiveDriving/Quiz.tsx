...
import { sampleDefensiveQuizSession } from "../../data/questions/generateDefensiveQuestions";
import { getTopicById, SEVERITY_LABEL, SEVERITY_COLOR } from "../../data/defensive";
...
export default function Quiz() {
  ...
  const startQuiz = () => {
    setQuestions(sampleDefensiveQuizSession(length));
    setIndex(0);
    setScore(0);
    setAnswered(false);
    setLastCorrect(null);
    ...
    return (
      <div className="flex flex-col gap-5 px-4 py-6">
        <div className="rounded-2xl border border-asphalt-700 bg-asphalt-900 p-5 text-center">
          ...
          <h2 className="mt-3 text-lg font-bold text-signal-white">Quiz de Direção Defensiva</h2>
          <p className="mt-2 text-sm text-asphalt-300">
            ...
            você jogar, mais situações diferentes vai revisar.
          </p>
        </div>
        ...
        <button
          type="button"
          onClick={startQuiz}
          className="w-full rounded-xl bg-signal-blue py-3.5 text-sm font-bold text-signal-white active:scale-[0.98]"
        >
          Começar quiz
        </button>
      </div>
  ...
  const topic = getTopicById(question.refId);
    ...
    <div className="flex flex-col gap-4 px-4 py-5">
      ...
      {topic && (
        <div className="flex flex-col items-center gap-2 rounded-xl bg-asphalt-900 py-6">
          ...
          <p className="px-4 text-center text-sm font-semibold text-signal-white">{topic.title}</p>
          {topic.severity && (
            <span
      ...
      {answered && (
        <div className="space-y-2">
          {lastCorrect === false && topic && (
            <p className="rounded-xl bg-asphalt-900 px-3.5 py-2.5 text-xs text-asphalt-300">
              💡 <span className="font-semibold text-signal-white">{topic.title}:</span>{" "}
              {topic.correctAction}
            </p>
          )}
          <button
